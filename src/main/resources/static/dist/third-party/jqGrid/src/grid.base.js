		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					if (ts.firstElementChild) {
						var resulthtml = ts.firstElementChild.innerHTML + rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
						$(ts.firstElementChild).empty().append(resulthtml);
					} else {
						$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join(''));
					}
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						...
						//FROS
						$(ts).on('jqGridAfterGridComplete.pg',function(){
							if(ts.p.multiselect == true){
								$('.jqgrow:first',$(ts)).addClass('ui-state-selected').attr('tabindex','0');
							}else{
								//single select
								if(ts.p.selrow != null){
									var r = $(ts).jqGrid('getGridRowById', ts.p.selrow);
									$(r).attr('tabindex','0');
								}else{
									$(ts).jqGrid('setSelection', 1, true, event);
								}
							}
							//兼容IE
							...
							$(ts).off('jqGridAfterGridComplete.pg');
						});
						populate();
						return false;
					}
...
//					$(ts).jqGrid("setSelection", ri ,true,e);//FROS
					ts.p.editTab = false;
				} else if (td.length > 0) {
					var _ri = ptr[0].rowIndex;
//					$(ts).jqGrid("setSelection", ri ,true,e);
					try {
						ts.p.editTab = false;
						$(ts).jqGrid("editCell",_ri,ci,true);
					} catch (_) {}
				}
			}
			if (!cSel) {
				...
				if (  ia === -1 ){
					if(pt.className !== "ui-subgrid") { 
						...
						if($t.p.cellEdit !== true) {
							$('.jqgrow[tabindex=0]',$($t)).attr('tabindex','-1');
							$(pt).attr('tabindex','0');
							$($t).attr('tabindex','-1');
						}
					}
		...
		return this.each(function(){
			...
			$($t).keydown(function(event){
				...
				if(target) {
					...
					var multiselect = $t.p.multiselect;
					if(event.keyCode === 37 || event.keyCode === 38 || event.keyCode === 39 || event.keyCode === 40){
						...
						if(event.keyCode === 38 ){
							...
							if(multiselect == true){
								$(target).removeClass('ui-state-selected').attr("tabindex","-1");
								var pt=$($t).jqGrid('getGridRowById', id);
								$(pt).addClass('ui-state-selected').attr("tabindex","0");
								$($t).attr('tabindex','-1');
								//scrollTop
					...
					//check if enter was pressed on a grid or treegrid node
						...
						if($.isFunction(o.onSpace)) {
							if(multiselect == true){
								o.onSpace.call($t, target.id,event);
							}else{
