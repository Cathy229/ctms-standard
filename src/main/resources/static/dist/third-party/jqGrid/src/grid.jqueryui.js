...
//			"jquery-ui/sortable",
			"jqueryui",
			"fros/widgets/jqGrid/plugins/ui.multiselect"		
		], factory );
	} else {
		// Browser globals
		factory( jQuery );
		...
		opts = $.extend({
			...
			apply_perm : function() {
				...
				$('option[selected]',select).each(function() { 
					...
					colNamesArr.push($(this).attr('colName'));
				});
				//FROS-253
                if (opts.savePerm) {
                    opts.savePerm.call(self, colNamesArr);
                }
...
//				self.jqGrid("setGridWidth", self[0].p.tblwidth, self[0].p.shrinkToFit);
		...
		$.each(colModel, function(i) {
			...
			//FROS
			select.append("<option colName='"+this.name+"' value='"+i+"' "+
...
//		multiselectData.availableList.css("height", listHeight);
		//FROS
		multiselectData.selectedList.height(Math.max(multiselectData.selectedContainer.height() - multiselectData.selectedActions.outerHeight() - 1, 1));
		multiselectData.availableList.height(Math.max(multiselectData.availableContainer.height() - multiselectData.availableActions.outerHeight() - 1, 1));
	},
