...
//			$t.p.selrow = $t.rows[iRow].id;
			...
			if($t.p.multiselect != true){
				$($t).jqGrid("setSelection", $t.rows[iRow].id ,true,null);
			}
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				...
				$(cc).html("").append(elc).attr("tabindex","-1");//FROS
				...
				window.setTimeout(function () { 
					//FROS
					if($(elc)[0].tagName.toLowerCase() == "span"){
						if($('.tofocus',$(elc))[0] != undefined){
							$('.tofocus',$(elc))[0].focus();
						}
					}else{
						$(elc).focus();
				...
				if($(elc)[0].tagName.toLowerCase() == "span"){
					$('.tofocus',$(elc)).focusout(function(event){
						//true的时候不save，否则无法编辑
						if($t.p.editout !== true){
							$($t).jqGrid("saveCell",$t.p.savedRow[0].id,$t.p.savedRow[0].ic,false);
						}
					});
				}else{
					$(elc).focusout(function(event){
						if($t.p.editout !== true){
							$($t).jqGrid("saveCell",$t.p.savedRow[0].id,$t.p.savedRow[0].ic,false);
						}
					});
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					if (e.keyCode === 27) {
						if($("input.hasDatepicker",cc).length >0) {
							...
							if($(this).hasClass('suggestBox')) {
								$(this).templateac("closeContainer");
							}
							$(this).addClass('cannotBlur');
							$(this).removeClass('canBlur');
							$($t).jqGrid("restoreCell",iRow,iCol);
							$(this).addClass('canBlur');
							$(this).removeClass('cannotBlur');
						}
					} //ESC
					if (e.keyCode === 13 && !e.shiftKey) {
						if($(e.target).hasClass('suggestBox') && $(e.target).templateac("isContainerOpen")){
							//hasSelected
							if($(this).templateac("hasSelected")){
								return false;
							}else{
								$(this).templateac("closeContainer");
							}
				...
				//FROS
				$("input, select, textarea",cc).bind("keyup",function(e) {
					if (e.keyCode === 16) {
						$t.p.shiftKey = false;
					}
				});
	...
	restoreCell : function(iRow, iCol) {
		return this.each(function(){
			...
			//此timeout事件在save的timeout之后
			window.setTimeout(function () {
				//true的时候说明是因为编辑时restore
				if($t.p.editout !==true){
	...
	GridNav : function() {
		return this.each(function () {
			...
			$t.p.knv = $t.p.id + "_kn";
			var selection = $("<div style='position:fixed;top:0px;width:1px;height:1px;' tabindex='-1'><div tabindex='-1' style='width:1px;height:1px;' id='"+$t.p.knv+"'></div></div>"),
			i, kdir;
			...
			.keydown(function (e){
				...
				switch (kdir) {
					...
					case 32:
						if($t.p.multiselect == true){
							var rowid = $t.rows[$t.p.iRow].id;
							$($t).jqGrid("setSelection",rowid,true,e);
						}
					break;
					default :
						return true;
				...
				if($t.p.readyEdit == true){
					$t.p.readyEdit = false;
				}else{
					$('.selected-row',$t).removeClass('selected-row').removeClass('ui-state-hover').attr('tabindex','-1');
					$('.edit-cell',$t).removeClass('edit-cell').removeClass('ui-state-highlight');
				}
				window.setTimeout(function(){
					$($t).attr('tabindex','0');
					$t.p.editTab = true;
