...
function datepicker_getZindex( elem ) {
	...
	while ( elem.length && elem[ 0 ] !== document ) {
		...
		if ( position === "absolute" || position === "relative" || position === "fixed" ) {
			...
			value = parseInt( elem.css( "zIndex" ), 10 );
			if ( !isNaN( value ) && value !== 0 ) {
				return value + datepickerZindexBase;
			}
		}
		elem = elem.parent();
	}
...
//			keypress(this._doKeyPress).keyup(this._doKeyUp);
	...
	_attachments: function(input, inst) {
		...
		//FROS
		if (inst.trigger && !inst.trigger.hasClass("dt-trigger")) {
			inst.trigger.remove();
		}
		...
		if (showOn === "button" || showOn === "both") { // pop-up date picker when button clicked
			...
			//FROS
			if(input.next().hasClass("dt-trigger") || input.next().hasClass("form-control-feedback")){
				inst.trigger = input.next();
			}else{
			...
			inst.trigger.off('click.openDP');
			inst.trigger.on('click.openDP',function() {
...
//		return inst.input && inst.input.is( ":visible" ) && !inst.input.is( ":disabled" ) && !inst.input.is( ":focus" );
	...
	_attachHandlers: function(inst) {
		...
		inst.dpDiv.find("[data-handler]").map(function () {
			var handler = {
				prev: function () {
					$.datepicker._adjustDate(inst.input, -stepMonths, "M");//FROS传递DOM实例
				},
				next: function () {
