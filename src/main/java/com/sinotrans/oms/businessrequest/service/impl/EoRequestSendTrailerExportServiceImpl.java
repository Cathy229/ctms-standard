...
*/
@Service
public class EoRequestSendTrailerExportServiceImpl extends BaseAspect implements EoRequestSendTrailerExportService {
	...
	@Override
	public Map<String,String> assembleExportTrailerEntrust(Long[] eoetIds, Long userId) throws WriteException, IOException {
		...
		for (int i = 0; i < eoetIds.length; i++) {
			...
			map = this.sendEmailAfterExportTrailer(entrustTrailerMap);
		}
		return map;
	}

	@Override
	public Map<String,String> sendEmailAfterExportTrailer(Map entrustTrailerMap) throws WriteException, IOException {
		Map<String,String> map = new HashMap<>();
		//创建Excel路径
...
//           	writableCellFormat.setAlignment(Alignment.CENTRE);
            	writableCellFormatBottom.setVerticalAlignment(VerticalAlignment.CENTRE); 
            ...
            if (null != entrustTrailerMap.get("eorbBulkBillNo")) {
            	...
            	eoetBlNo = entrustTrailerMap.get("eoetBlNo") == null ? "" : entrustTrailerMap.get("eoetBlNo").toString();
            }
...
//		sendMailUtil.setAddress("lmsupport@sinotrans.com", email, emailTitle);
	...
	private Map<String, Object> assembleTrailerEntrust(EoOrderModel eoOrderModel, List<EoRequestBookingModel> eoRequestBookingModels, 
		...
		if (null != eoRequestTrailerModel) {
			...
			String eoetEbrgNameCn = eoRequestTrailerModel.getEoetEbrgNameCn();//提单号
			//将查询到的信息放到map中
