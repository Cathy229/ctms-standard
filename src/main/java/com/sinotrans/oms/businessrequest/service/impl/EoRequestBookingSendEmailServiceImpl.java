...
*/
@Service
public class EoRequestBookingSendEmailServiceImpl extends BaseAspect implements EoRequestBookingSendEmailService {
	...
	public void assembleIssuedBookingEntrust(Long[] eorbIds, Long userId,String accessToken) throws WriteException, IOException {
		...
		for (int i = 0; i < eorbIds.length; i++) {
			...
			if (EmptyUtils.isNotEmpty(ebcuCustomerNo) && EmptyUtils.isNotEmpty(sendToEdiBookingEbcuCode) && sendToEdiBookingEbcuCode.equals(ebcuCustomerNo)) {//订舱代理的客户代码与系统中配置的控制参数一致，则认为是华东订舱网
				//下达组装报文，并且发送报文给EDI
				//直接对接订舱网的生产环境，生产环境发布时放开 
				this.ediBookingService.sendBookingBRToEAI(eorbId);
				//将组装好的拖车委托书信息保存到excel中并发送邮件
				this.sendEmailAfterIssuedBooking(entrustBookingMap);
	...
	private Map<String, Object> assembleBookingEntrust(EoRequestBookingModel eoRequestBookingModel,
		...
		if (null != eoRequestBookingModel) {
			...
			String eorbTransferPortName = eoRequestBookingModel.getEorbReceiptPlaceName();//卸货地
			...
			String eorbCarrierBookingNo = eoRequestBookingModel.getEorbCarrierBookingNo();//订舱号
			...
			Long eorbEscoId = eoRequestBookingModel.getEorbEscoId();
...
//				mobile = esUserModel.getEsusMobile();//手机
	...
	@Override
	public void sendEmailAfterIssuedBooking(Map<String, Object> entrustBookingMap) throws WriteException, IOException {
		//创建Excel路径
//	    String path = EoRequestBookingSendEmailServiceImpl.class.getClassLoader().getResource("com/sinotrans/oms/file/booking.XLS").getPath();
...
//			is = new FileInputStream(excelpath);				  //创建一个文件流，读入Excel文件
            ...
            for (int i = 0; i < sheet_size; i++) {
            	...
            	sheet.addCell(new Label(4,1, "订单号:" + eoOrderNo,wcfRight));//订单号
            	sheet.addCell(new Label(4,2, "",wcfRight));//
            	sheet.addCell(new Label(4,3, "",wcfRight));//

            	String eorbBookingContractNo = (String) entrustBookingMap.get("eorbBookingContractNo");//订舱协议号
            	String eorbCarrierBookingNo = (String) entrustBookingMap.get("eorbCarrierBookingNo");//预约号
                Label labelTo =   new   Label(2,4,eorbBookingContractNo,wcf); 
...
//		wcfRight.setAlignment(Alignment.CENTRE);   //设置居中对齐 
//		titleLeft.setAlignment(Alignment.CENTRE);   //设置居中对齐 
//		wcfButtom.setAlignment(Alignment.CENTRE);   //设置居中对齐 
//		wcfRightMedium.setAlignment(Alignment.CENTRE);  //设置居中对齐 
        ...
        WritableCellFormat wcf = new   WritableCellFormat(wf); 
//        wcf.setVerticalAlignment(VerticalAlignment.CENTRE);   //设置居中对齐
		wcf.setBorder(jxl.format.Border.ALL,jxl.format.BorderLineStyle.MEDIUM);
		//Total
...
//        wcfTotal.setAlignment(Alignment.CENTRE);   //设置居中对齐
